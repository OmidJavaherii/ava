...
const PageNav = (props: {currentPage: number; items: number; onChange: Function}) => {
	...
	//this function calculates the number of pages based on the overall item count
	const getPages = () => {
		const pages = Math.floor(props.items / 10) + (props.items % 10 > 0 ? 1 : 0);
	...
	const getRange = (): Array<number> => {
		...
		if (props.currentPage === lastPage - 1) return [lastPage - 5, lastPage - 1];
		if (props.currentPage === lastPage - 2) return [lastPage - 5, lastPage - 1];
	...
	return (
		...
		>
			<span
				...
				onClick={() => props.currentPage > 1 && props.onChange(props.currentPage - 1)}
			>
				{'>'}
			</span>
			<span
					...
					{getPages()
						...
						.map((num, k) => (
							<span
								key={k}
								className={`${
									num === props.currentPage && 'bg-ava-green text-white'
								} cursor flex h-8 w-8 cursor-pointer items-center justify-center rounded-full hover:bg-slate-200`}
			...
			{lastPage > 1 && (
				<span
					className={`${
						lastPage === props.currentPage && 'bg-ava-green text-white'
					} cursor flex h-8 w-8 cursor-pointer items-center justify-center rounded-full hover:bg-slate-200`}
			...
			<span
				className="cursor flex h-8 w-8 cursor-pointer items-center justify-center rounded-full hover:bg-slate-200"
				onClick={() => props.currentPage < lastPage && props.onChange(props.currentPage + 1)}
			>
				{'<'}
			</span>
		</div>
